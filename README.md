...
|---|---|---|---|
| **DimCrochet** | crochet round & motif charts | v0.9.2 | `ghcr.io/daimaah/dimcrochet` |
| **DimKnit** | knitting charts | v0.3.0 | `ghcr.io/daimaah/dimknit` |
...
`core-vX.Y.Z` (currently core v0.3.0). DimCraft itself has no version number:
...
- **DimCrochet** is versioned `vX.Y.Z` (currently **v0.9.2**, numbering started at v0.5.0 — no
  license dictates a scheme): git tags `v0.9.2`, image `ghcr.io/daimaah/dimcrochet:v0.9.2`,
  summarized in the [app changelog](apps/dimcrochet/CHANGELOG.md).
- **DimKnit** is versioned `dimknit-vX.Y.Z` (currently **v0.3.0**): git tags
  `dimknit-v0.3.0`, image `ghcr.io/daimaah/dimknit:dimknit-v0.3.0`, summarized in
...
- **DimCraft core** — the shared chart-editor kernel, the short-link sidecar and the shared
  build/deploy infra — is versioned `core-vX.Y.Z` (currently **core v0.3.0**): bumped only when
