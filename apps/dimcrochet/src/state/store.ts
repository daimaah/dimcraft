...
import type {
  ...
  Tool,
  Vec,
} from '@dimcraft/core/model/types'
import { createEmptyDoc, sanitizeDoc, uid } from '@dimcraft/core/model/doc'
import { guideCenter, guideSample } from '@dimcraft/core/geometry/guides'
import { placeEvenly } from '@dimcraft/core/geometry/placeEvenly'
import { legendItems } from '@dimcraft/core/geometry/legend'
import { buildClipboard, loadClipboard, pasteClipboardInto, saveClipboard } from '@dimcraft/core/model/clipboard'
...
import { longestSegment } from '@dimcraft/core/geometry/handles'
  ...
  return {
    ...
    alignSelection: (axis, mode) =>
      set((st) => {
        ...
        const defMap = getDefMap(st.doc)
        const boxes = new Map<string, import('@dimcraft/core/geometry/transform').BBox>()
        for (const p of sel) {
          const def = defMap.get(p.symbolId)
