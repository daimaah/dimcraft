# Changelog — DimCrochet

All notable changes to the DimCrochet app are documented here. The format
loosely follows [Keep a Changelog](https://keepachangelog.com/); the project
is not yet 1.0, so versions started at 0.5.0 — no license requires any
particular numbering scheme. This file summarizes the (public) git commit
history; the repository log remains the complete, transparent record.

Changes to the shared chart-editor kernel, the short-link sidecar and the
build/deploy infrastructure are versioned separately as the **DimCraft core**
and summarized in the [repository-root CHANGELOG](../../CHANGELOG.md).
...
- **Legend spacing** — the legend title no longer crowds the first row of
  entries (shared-rendering fix, also in DimKnit).
