...
import { applySetToDefs, resolveSet } from '@dimcraft/core/symbols/sets'
import { BUILTIN_SETS } from '../src/symbols/sets'
import { builtInDefsFor, getDefMap } from '@dimcraft/core/symbols/registry'
import { TERMINOLOGY_PRESETS, LADDER_IDS } from '../src/symbols/terminology'
import { readProjectFile } from '@dimcraft/core/export/projectFile'
import { createEmptyDoc } from '@dimcraft/core/model/doc'
import type { ChartDoc, CustomSet } from '@dimcraft/core/model/types'

describe('symbol sets', () => {
  it('golden fixture: chart referencing a bundled set and one of its added symbols imports', async () => {
    const dir = fileURLToPath(new URL('../../../packages/core/tests/fixtures/', import.meta.url))
    const text = readFileSync(`${dir}v1-commons-chart.dimcrochet.json`, 'utf8')
    const parsed = await readProjectFile(new File([text], 'v1-commons-chart.dimcrochet.json', { type: 'application/json' }))
