...
import { getDefMap, symbolInner } from '@dimcraft/core/symbols/registry'
...
const SEEN_MINE_KEY = 'dimknit.seenMine'
const SORT_KEY = 'dimknit.designsSort'

type Tab = 'starters' | 'mine'

/** Home view: learn-with-starters, your designs, backup/restore — same
 *  layout and styling as the DimCrochet gallery, knit content and theme. */
export function Gallery() {
  ...
  const starterDocs = useMemo(
    () => KNIT_STARTERS.map((s) => ({ def: s, doc: s.make() })),
    [],
  )
  ...
  const importFiles = async (files: File[]) => {
    for (const f of files) {
      ...
      if (!parsed) {
        window.alert(`Could not read ${f.name}. Expected a DimKnit chart or backup export.`)
        continue
      }
      if (parsed.type === 'pack') {
  ...
  const create = (name: string, doc?: ChartDoc) => {
    useStore.getState().newProject(name, doc ?? createStarter('stockinette'))
  ...
  return (
    ...
    >
      ...
      {activeTab === 'starters' && (
        <>
          <p className="hint starters-intro">
            New to knitting? Open a starter in increasing difficulty, then hit ▶ Follow to walk the rows
            stitch by stitch — right-side rows run right-to-left, wrong-side rows reverse.
          </p>
          <div className="starter-grid" data-testid="starter-grid">
            {starterDocs.map(({ def, doc }) => (
              <article
                ...
                onClick={() => create(def.name, doc)}
                title={`Open “${def.name}” as a new chart`}
              >
                <MiniChart doc={doc} />
                <div className="starter-meta">
                  <div className="starter-title-row">
                    <strong>{def.name}</strong>
                    <span className={`level-chip level-${def.levelNum}`}>{def.level}</span>
                  </div>
      ...
      {activeTab === 'mine' && (
        <div className="mine-toolbar" data-testid="mine-toolbar">
          <label className="mine-sort">
            <span>Sort:</span>
            <select value={sort} onChange={(e) => setSort(e.target.value as DesignsSort)} data-testid="mine-sort">
              {DESIGNS_SORTS.map((o) => (
      ...
      {activeTab === 'mine' && (
        <div className="cards-list">
          {sortProjects(projects ?? [], sort).map((rec) => (
            <article key={rec.id} className="project-card" onDoubleClick={() => useStore.getState().openProject(rec)}>
              <div className="project-main" onClick={() => useStore.getState().openProject(rec)}>
                ...
                <div className="project-meta">
                  ...
                  <span className="hint" title={`Last edited ${editedTimestamp(rec.updatedAt, !clock24h)}`}>
                    {rec.doc.placements.length} stitches · edited {editedLabel(rec.updatedAt, nowTick, !clock24h)}
                  </span>
                </div>
              </div>
      ...
      <footer className="gallery-foot">
        <p className="hint">
          DimKnit v{__APP_VERSION__}{__GIT_COMMIT__ ? ` (${__GIT_COMMIT__})` : ''}, core {__CORE_VERSION__} — charts
          are stored only in this browser. Drop a .dimknit export or backup anywhere on this page to import it —
          purely client-side, nothing is uploaded. DimKnit is open source (MIT).
        </p>
        <div className="gallery-foot-actions">
          <button className="btn" data-testid="backup-download" title="Download all charts and settings as one file" onClick={() => void downloadBackup()}>
            ⭳ Backup everything
          </button>
          <label className="btn" data-testid="backup-restore" title="Restore a backup file: adds charts, replaces charts with the same id, applies settings">
            ⭱ Restore backup…
            <input
              type="file"
